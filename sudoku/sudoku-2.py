...
#    disposible = input()
    ...
    if iteration > 1000 and iteration % 500 == 0 :
        ...
        print('currently at {} percentage completion'.format(check_progress()))
        print('\n')
    if iteration > 30 and (iteration -20) % 30 == 0 :
        guesses += 1
        save_points = copy.deepcopy(points)
        save_iter = iteration
        for point in points:
            if len(point.possible) == 2:
                ...
                if starting_validity_check() == False:
                    print('wrong guess')
                    points = copy.deepcopy(save_points)
                    if points == save_points: print('loaded the last checkpoint')
                    iteration = save_iter + 1
                    set_certain(point.y,point.x,other_option)
                elif starting_validity_check() == True:
                    print('right guess')
